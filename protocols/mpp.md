...
- **Intents** — the type of payment. `charge` (one-time) and `subscription` (recurring, merged 2026-07-29) are on main; Tempo and Lightning also have merged `session` intents. The `authorize` intent PR was closed without merging.
- **Methods** — 10 payment-method specs on main: card, evm (unified charge, merged 2026-04-17), hedera (merged 2026-05-14), lightning (charge + session), nearintents, solana, stellar, stripe, tempo (charge + session), usdc. Initia still open; TON NanoPay was closed without merge.
...
- Multi-rail settlement (10 payment-method specs on main)
...
- Pre-authorization with delayed capture — the `authorize` intent PR (#226) was closed without merging
...
**Recently merged:** Subscription intent + Stripe and Tempo implementations ([#230](https://github.com/tempoxyz/mpp-specs/pull/230), 2026-07-29), Near Intents and USDC payment methods, unified EVM charge (#213, 2026-04-17), Hedera charge (2026-05-14), challenge-binding security guidance (#233, 2026-04-16), Stripe Connect settlement docs (2026-05-20). The `authorize` intent ([#226](https://github.com/tempoxyz/mpp-specs/pull/226)) and TON NanoPay (#214) were both closed without merge.

**Note on "machine-token":** support for machine-token charge payments landed in the Rust SDK ([mpp-rs #385](https://github.com/tempoxyz/mpp-rs/pull/385), merged 2026-08-14) under the existing `tempo` method. It is not a separate payment-method spec — the string `machine-token` does not appear anywhere in `mpp-specs` on main.
